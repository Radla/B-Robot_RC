...
volatile byte cppm_rdy = 0;
volatile byte err_flag = 0;
...
void setup()
{
  ...
  pinMode(A3, OUTPUT);   // Scope trigger
  digitalWrite(A3, LOW);
...
#endif
//  SerialUSB.println("BROBOT by JJROBOTS v3.04");
...
//    if (OSCpage == 1)   // Get commands from user (PAGE1 are user commands: throttle, steering...)
    if (true)   // Get commands from user (PAGE1 are user commands: throttle, steering...)
    {
...
//        throttle = (OSCfader[0] - 0.5) * max_throttle;
        ...
        if (steering > 0)
          ...
          steering = (-steering * steering + 0.5 * steering) * 260;
      }
...
//    target_angle = constrain(target_angle, -max_target_angle, max_target_angle); // limited output
    target_angle = constrain(target_angle, -24, 24); // limited output
...
//    if (OSCpush[0])     // If we press the SERVO button we start to move
    if (ch_svo == 1)     // If we press the SERVO button we start to move
      angle_ready = 82;
    else
